...
@test "link user claude creates per-entry symlink for claude-compatible skill" {
  ...
  [ "$(readlink "$HOME/.claude/skills/alpha")" = "$TOOLKIT_ROOT/skills/alpha" ]
}
...
@test "link user codex skips skill not tagged for codex" {
  run "$BATS_TEST_DIRNAME/../../bin/agent-toolkit" link user codex --toolkit-repo "$TOOLKIT_ROOT"
  [ "$status" -eq 0 ]
...
@test "link user claude removes stale symlink when harnesses change" {
  ...
  sed -i.bak 's/- claude/- codex/' "$TOOLKIT_ROOT/skills/alpha/SKILL.md" && rm "$TOOLKIT_ROOT/skills/alpha/SKILL.md.bak"
  run "$BATS_TEST_DIRNAME/../../bin/agent-toolkit" link user claude --toolkit-repo "$TOOLKIT_ROOT"
  [ "$status" -eq 0 ]
  [ ! -L "$HOME/.claude/skills/alpha" ]
}
...
@test "link user claude --dry-run does not create symlink" {
  run "$BATS_TEST_DIRNAME/../../bin/agent-toolkit" link user claude --toolkit-repo "$TOOLKIT_ROOT" --dry-run
  [ "$status" -eq 0 ]
...
@test "link user claude emits a header on stderr" {
  run bash -c "'$BATS_TEST_DIRNAME/../../bin/agent-toolkit' link user claude --toolkit-repo '$TOOLKIT_ROOT' 2>&1 >/dev/null"
  [ "$status" -eq 0 ]
  [[ "$output" == *"Linking"* ]]
}
...
@test "link --dry-run summary mentions pending changes" {
  run bash -c "'$BATS_TEST_DIRNAME/../../bin/agent-toolkit' link user claude --toolkit-repo '$TOOLKIT_ROOT' --dry-run 2>&1 >/dev/null"
  [ "$status" -eq 0 ]
  [[ "$output" == *"pending"* ]] || [[ "$output" == *"Nothing to change"* ]]
}

@test "link with AGENT_TOOLKIT_QUIET=1 only emits would-link/etc on stdout" {
  run bash -c "AGENT_TOOLKIT_QUIET=1 '$BATS_TEST_DIRNAME/../../bin/agent-toolkit' link user claude --toolkit-repo '$TOOLKIT_ROOT' --dry-run 2>&1"
  [ "$status" -eq 0 ]
  [[ "$output" != *"Linking"* ]]
  [[ "$output" == *"would-link"* ]]
}
