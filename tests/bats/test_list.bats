...
@test "list emits a header and summary on stderr" {
  run bash -c "'$BATS_TEST_DIRNAME/../../bin/agent-toolkit' list --toolkit-repo '$TOOLKIT_ROOT' 2>&1 >/dev/null"
  [ "$status" -eq 0 ]
...
@test "list is quiet under AGENT_TOOLKIT_QUIET=1" {
  run bash -c "AGENT_TOOLKIT_QUIET=1 '$BATS_TEST_DIRNAME/../../bin/agent-toolkit' list --toolkit-repo '$TOOLKIT_ROOT' 2>&1 >/dev/null"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
...
plugins: []
EOF
  run "$BATS_TEST_DIRNAME/../../bin/agent-toolkit" list --format=json --toolkit-repo "$TOOLKIT_ROOT"
  [ "$status" -eq 0 ]
  echo "$output" | uv run python -c "import json,sys; d=json.load(sys.stdin); assert d['toolkit_root']=='$TOOLKIT_ROOT'; assert any(a['slug']=='alpha' for a in d['assets']); cells=[c for a in d['assets'] if a['slug']=='alpha' for c in a['cells']]; assert any(c['harness']=='claude' and c['scope']=='user' and c['status']=='linked' for c in cells), cells"
}
