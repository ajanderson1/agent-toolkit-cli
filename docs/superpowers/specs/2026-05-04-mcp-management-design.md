...
| Project-scope `.mcp.json` is absent | A project-scope MCP link is a **silent no-op if `.mcp.json` doesn't pre-exist** (the v2.3.0 toolkit audit recorded this exact failure mode for `mcp × claude`: `ClaudeAdapter.config_target()` returned `None` when the file was absent, so the link did nothing without reporting). The Claude config-file adapter MUST create the file with the valid empty shape `{"mcpServers": {}}` and then upsert, never short-circuit on absence. |
| `.mcp.json` contains a bare `{}` (no `mcpServers` key) | Claude Code expects `{"mcpServers": {...}}`; a bare `{}` **fails Claude's validation/parse**. An empty `{}` file is a common placeholder (e.g. created to satisfy the "must pre-exist" workflow above). The adapter's round-trip read MUST normalise a `{}` or missing-`mcpServers` document to `{"mcpServers": {}}` before upsert — give it the valid empty shape rather than erroring or writing through the invalid bare `{}`. |

> **Note — the empty-`{}` parse error (captured 2026-06-07).** The repo's own docs note this exact failure mode: a project-scope MCP link is a "silent no-op if `.mcp.json` doesn't pre-exist", so an empty `{}` file is likely created as a placeholder for that workflow. The fix: Claude Code expects `{"mcpServers": {...}}`; an empty `{}` fails validation. With no project MCP servers defined, the right fix is to give the file the valid empty shape `{"mcpServers": {}}` — `link`'s round-trip read must normalise both the absent-file and bare-`{}` cases to that shape before upserting, never write through a bare `{}` and never short-circuit on absence.
