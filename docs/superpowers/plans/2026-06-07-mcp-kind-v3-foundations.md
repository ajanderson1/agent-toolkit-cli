...
Per the spec's strategy table (Pi cell CORRECTED 2026-06-13 — see note 2): Claude writes `mcpServers.<name>` to `.mcp.json` (project) / `~/.claude.json` (user); **Pi writes to `<project>/.mcp.json` (the SHARED "standard" file, same as claude-code project) / `~/.pi/agent/mcp.json` (Pi-owned global override, honors `$PI_CODING_AGENT_DIR`)**; OpenCode writes `mcp.<name>` to `<project>/opencode.json` / `~/.config/opencode/opencode.json` and translates the inner config to OpenCode's native shape. This task implements Claude + Pi (identical `mcpServers` shape); OpenCode's translation is its own per-cell branch. **Because Pi-project and claude-code-project both resolve to `.mcp.json`, the upsert into that shared doc must be idempotent by server name (installing for both at project scope writes the file once), and `doctor`/`list` must de-dup the shared file across the harnesses that read it.**
...
2. **Pi targets — VERIFIED 2026-06-13, open item CLOSED.** Pi has NO native MCP; it is provided by the **`pi-mcp-adapter` extension**. Empirical probes (`pi -p` with each assumed path present) returned `NO_MCP_SERVERS` — the old cells were wrong at the mechanism level. The adapter reads `~/.config/mcp/mcp.json` (shared) → `~/.pi/agent/mcp.json` (Pi-global) → `.mcp.json` (shared project) → `.pi/mcp.json` (Pi project). **Corrected cell (AJ):** Pi user → `~/.pi/agent/mcp.json` (honor `$PI_CODING_AGENT_DIR`); Pi project → the shared `.mcp.json`. No further launch-Pi verification step is needed — the cell is settled. (The shared-`.mcp.json` overlap with claude-code is handled by the idempotent-by-name upsert + doctor/list de-dup, per the strategy-table note above.)
3. **Installed-harness sentinel (spec failure-mode table).** Each cell carries a `sentinel`: `~/.claude/` (claude-code), `~/.codex/` (codex), `~/.config/opencode/` (opencode). **Pi is ADAPTER-GATED:** its sentinel is the `pi-mcp-adapter` extension at `~/.pi/agent/npm/node_modules/pi-mcp-adapter` (NOT just `~/.pi/`) — an MCP written for Pi when the adapter is absent is never read (silent-green-breakage). When a sentinel is absent, print `"<harness> does not appear to be installed (<sentinel> missing); skipping"` and skip — exit 0. (NOTE: Pi *project* scope writes the shared `.mcp.json` regardless of the Pi adapter, since that file serves every host; the adapter-gating applies to the Pi *user* target `~/.pi/agent/mcp.json`.) NEVER unconditionally `mkdir -p` a harness config tree for a harness that is not installed. `atomic_write_text` stays generic (no sentinel logic in the write primitive).
...
- `list_cmd.py` — `read_only=True`; prints library slugs with their resolved version (or `floating`) and, per slug, which harnesses are installed at the resolved scope (via `get_adapter(h).is_installed`). When the scope lock's `pin` lags the library entry's version, show it: `<pin> (library: <version> — stale)`. **Also surfaces native/manual installs:** scan the resolved-scope harness configs for MCP entries NOT in the lock and display them as `[!] unmanaged (<harness>)` — visible, never touched (manage-by-name boundary; the spec's `[!]` state). A future `doctor` adopt flow (instructions-kind #337 precedent) may offer to claim them; out of scope here.
...
- `update_cmd.py` — **advance the float, explicitly and greedily** (`mcp update <slug>` — NO scope flag; one version of each MCP exists, the library entry, and update propagates it to every projection it can reach):
  1. **Re-resolve** the entry's source (same per-method resolution as `add`); if the resolution moved, rewrite the library entry (config.json args + sidecar `resolved_version`). `--url` entries: nothing to resolve; `--local`: refresh the recorded HEAD SHA.
  2. **Re-upsert every reachable locked projection**: every (harness) the GLOBAL lock lists for the slug, plus every (harness) the CURRENT PROJECT's lock lists (when run inside a project). Upserts are idempotent; refresh `pin` on each re-upserted lock entry. No comparisons, no short-circuits — re-render and upsert unconditionally (adapters may skip the disk write when bytes are identical).

  Output: per-scope `<slug>: <old> → <new> (<harnesses>) [global|project]` with old/new read from the lock pins; `<slug>: up to date (<version>)` when nothing moved anywhere. **Honest limitation (direct-writes model, shim rejected):** OTHER projects' locks are not discoverable from here — a different project's `.mcp.json` keeps its baked version until you run `update` (or an idempotent re-`install`) inside it; its `doctor`/`list` flag the staleness meanwhile. Scope semantics: **the library answers WHICH version; the scope answers WHERE** — per-scope version pinning (holding one project at an older version on purpose) is an explicit v1 non-goal; do NOT half-implement it.
...
Also covered here: an **`update` flow test** — monkeypatch the version-resolution helper to return a bumped version, run `mcp update <slug>`, assert the library entry's `config.json` args + sidecar `resolved_version` were rewritten, every locked harness's installed entry shows the new version (re-upserted), the lock `pin` refreshed, and output contains `old → new`. A second case: resolver returns the same version AND projections match the entry → no writes, output `up to date`. A third case (greedy cross-scope propagation): seed the slug installed at BOTH global scope and the current project with old pins, resolver returns a bumped version → ONE flagless `mcp update <slug>` run from inside the project must rewrite the library entry AND re-upsert global + project projections, refreshing both locks' pins (per-scope `old → new` lines in output). A fourth case: an entry name present in the harness config but NOT in the lock must appear in `mcp list` output as `[!] unmanaged` and survive an update/uninstall of a managed sibling byte-/structurally intact. And an **`add` resolution-failure test** — monkeypatch the resolver to raise, assert the entry is stored floating (no version in args, no `resolved_version`) with a loud note, exit 0.
...
>
> 3. **Update/coexistence mechanics finalised (AJ, same-day mechanism discussion, round 3):** a **shim/launcher model** (stable `agent-toolkit-cli mcp run <slug>` stub in harness configs, dereferencing the library at spawn) was considered and **REJECTED — single point of failure** in every harness's session-spawn path. Final model: **direct writes** (configs carry the real resolved command; self-describing, work natively without the toolkit) + **greedy scope-less `update`** (library + global lock + current-project lock in one run; other projects catch up on visit, doctor flags meanwhile) + **manage-by-name coexistence** (native/hand-rolled entries shown as `[!] unmanaged` in `list`, never touched; lock = pure ownership registry; `pin` records the version baked per scope; doctor-adopt is a follow-up).
>
