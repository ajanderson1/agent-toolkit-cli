...
The single source of truth for which **(asset type × harness)** pairs are
supported and how each is projected. This file is machine-read by the CLI
(shipped in the wheel), guarded by parity tests, and the source the
human-friendly [matrix](../matrix.md) and per-harness pages are generated from
(`scripts/gen_harness_docs.py`). Edit this file, then regenerate the views.

## What this file covers

The toolkit has five [asset types](../glossary.md#asset-type). Three are
trivial to place and don't need a per-harness research table, so they are
**not** encoded here:

- **skills** — every harness in the catalog has a skills directory; the set
  lives in `src/agent_toolkit_cli/skill_agents.py`.
- **MCP servers** — config-injection into four harnesses (`claude-code`,
  `codex`, `opencode`, `pi`); the set lives in `gen_harness_docs.py::MCP_ADAPTERS`.
- **pi extensions** — Pi-only by definition.

The remaining two asset types **vary per harness** and each carries its own
researched support table here, in its own section. Both sections follow the
same shape — a one-line summary, the **mechanism vocabulary** that classifies
each row, the **support table** (machine-read), then **notes**:
...
(no such concept), or **unknown — no public evidence found**. See the
[glossary](../glossary.md#verdict) for how each renders in the matrix.

---

## Convention compliance — does the harness read our canonical placement?

For each asset type the toolkit adopts **one canonical placement** (the
convention). A harness **complies** when it reads that placement **natively** —
zero adapter work, just drop our canonical file and the harness finds it. A
**non-compliant** harness needs per-harness adapter work (a pointer symlink, a
translated/reshaped file, or its own slot) to be satisfied. This is the
"how much converges for free vs how much we adapt" view; the supported/gap/
unknown verdicts are in the two per-asset-type tables below.

Only three asset types have a *shared* convention to comply with. **MCP servers**
(config-injection into each harness's own config, by name) and **pi extensions**
(Pi-only) have no cross-harness canonical placement, so compliance doesn't apply.
...
The 15 non-compliant split into 7 that need a same-name pointer symlink
(`augment`, `claude-code`, `codebuddy`, `gemini-cli`, `iflow-cli`, `replit`,
`tabnine-cli`) and 8 with no pointer-satisfiable root file at all.

**`skills` — 14 compliant** (read `.agents/skills` directly):
`amp`, `antigravity`, `cline`, `codex`, `cursor`, `deepagents`, `dexto`,
`firebender`, `gemini-cli`, `github-copilot`, `kimi-cli`, `opencode`, `replit`,
`warp`. Source: `is_standard` (skills_dir == `.agents/skills`) in
`skill_agents.py`. Every other catalog harness gets its own skills directory.

**`agents` — 6 compliant** (read the shared `.claude/agents/` slot):
`claude-code`, `cortex`, `cursor`, `kode`, `neovate` at **both** scopes, plus
`devin` at **project scope only** (its global agents live elsewhere). Source:
`STANDARD_AGENT_READERS` in `agent_adapters/standard.py` — the per-scope SSOT a
single shared write satisfies. The other 22 supported harnesses each need their
own per-harness slot; the rest have no subagent concept. See the
[subagent notes](#notes-agent-subagent) for the convergence evidence.

> These counts are **derived** from the per-asset-type tables below plus the two
> code tables named above (`is_standard`, `STANDARD_AGENT_READERS`). They are
> maintained by hand here — if you change a verdict below or a code table, update
> this section too. (Not yet emitted by `gen_harness_docs.py`.)

---

## Instruction-file (`instructions` asset type) support — all harnesses

For every harness in the catalog: what file does it load by **default** (no
flags, no config) as its root project/global instruction context, and can a
per-harness pointer symlink to a canonical `AGENTS.md` satisfy it? Each
`symlink`-verdict row becomes one pointer (e.g. `CLAUDE.md → AGENTS.md`).
Per-harness "what I checked" evidence trails live in
`docs/agent-toolkit/research/instructions-fragments/`.

**Summary:** 39 native · 7 symlink · 4 unsupported (gap) · 2 unsupported (by design) · 2 unknown.

The 39 `native` readers cost zero adapter work — the canonical `AGENTS.md`
satisfies them as-is. The 7 `symlink` harnesses (alphabetical) are the only
ones needing a pointer: `augment`, `claude-code`, `codebuddy`, `gemini-cli`,
`iflow-cli`, `replit`, `tabnine-cli`.
...
This asset type has **one supported mechanism** (pointer symlinks only — no
`translate`, no `config_file`, no `dual-symlink`). The five-cell vocabulary:
...
- **`continue` is a `gap`, not unsupported.** Current upstream loads
  `.continue/rules/*.md` directories but no single root file → file-vs-directory
  rule → `gap`.
- **The "standard" set is large.** The 39 `native` readers ARE the "standard"
  column for this asset type — rendered in the TUI as informational (always
  satisfied, no toggle).
...
For every harness in the catalog, the `agent` (subagent) verdict and how it is
projected. Each `supported` row (mechanism =
`symlink`/`translate`/`config_file`/`config_file+folder`/`dual-symlink`) maps to
one adapter behaviour (`src/agent_toolkit_cli/skill_agents.py`, excluding the
synthetic `standard` entry). Per-harness "what I checked" evidence trails and v1
baseline deltas live in `docs/agent-toolkit/research/subagent-fragments/`.

**Summary:** 28 supported · 11 unsupported (gap) · 10 unsupported (by design) · 5 unknown.

The 28 supported harnesses, by mechanism:

- **symlink** (Claude-compatible markdown drop-in) — `augment`, `claude-code`,
  `codebuddy`, `command-code`, `cortex`, `cursor`, `droid`, `forgecode`,
  `junie`, `kode`, `neovate`, `pochi`, `qoder`, `rovodev`.
- **translate** (reshaped frontmatter / non-md format) — `devin`, `gemini-cli`,
  `github-copilot`, `kilo`, `kiro-cli`, `mistral-vibe`, `mux`, `opencode`,
  `qwen-code`.
- **config_file+folder** (registry-pointed) — `aider-desk`, `codex`, `dexto`,
  `firebender`.
- **dual-symlink** — `pi`.

### Mechanisms (agent / subagent)
...
### Notes (agent / subagent)

- **Mechanism normalization:** the research fragments labelled several
  Claude-compatible markdown drop-ins as `config_file+folder`; that was a
  loose reading of "reads a directory". In this matrix's vocabulary a
...
- **`codex`** requires an `[agents.<role>]` declaration in `config.toml`
  pointing at the TOML via `config_file=`, so it is `config_file+folder`, not
  pure `translate` — a translate-only adapter is insufficient.
- **`pi`** — the user-scope `~/.agents/` alias appears to be a *skills* path,
...
- **Cross-harness convergence (re-verified 2026-06-10):**
  ...
  - **Global (`~/.claude/agents/`):** `claude-code`, `kode`, `neovate`,
    `cortex`, and `cursor` — https://cursor.com/docs/subagents lists
    `~/.claude/agents/` (user) and `.claude/agents/` (project) as default
    subagent discovery locations, `.cursor/` winning name conflicts.
  ...
  - Near-misses that do **not** count: `droid` (one-time *import* command
    copying into `~/.factory/droids/`), `junie` (detects `.claude/agents/` and
    *suggests importing* into `.junie/agents/`), `kiro-cli` (documented as
    having no `.claude/` compat), `qwen-code` (extension-install-time conversion
    only). Evidence trails in the per-batch fragments.
  - A single write into `.claude/agents/` satisfies all readers at that scope.
    This single-shared-file approach is the **standard** agents projection:
    `agent install --harnesses standard` writes the one shared slot file, and
    ...
    the code table is what the CLI/TUI consult).
