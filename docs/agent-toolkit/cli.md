...
`agent doctor` also checks the standard slot (#361 finding families): `standard-slot-drift` (slot differs from the scope's canonical; fix re-seeds it), `cursor-shadow` (a stale pre-existing `.cursor/agents/<slug>.md` projection — cursor's own dir **wins** name conflicts, so it shadows the slot; remediate by uninstalling the old cursor projection, via doctor's removal fix or `agent uninstall <slug> --harnesses cursor`), `standard-slot-orphan` (tool-written slot file with no lock entry), `standard-slot-unmanaged` (hand-authored files in `.claude/agents/` — informational only, **never** auto-removed), and `standard-slot-dangling-sidecar` (stale `.attk` ownership sidecar without its slot file).
