"""`pi-extension push [slugs] [-g/-p] [--direct]` — push local commits upstream.

npm rows have nothing to push (no git clone). Store-owned rows follow the same
PR/direct path as `skill push` including the clean-gap fix (#280): a clean
working tree that is AHEAD of origin still has committed-but-unpushed work to
publish.
"""
...
\b
  agent-toolkit-cli pi-extension push                   # push all extensions
  agent-toolkit-cli pi-extension push my-ext            # push one extension
  agent-toolkit-cli pi-extension push --direct my-ext   # push straight to ref
""")
...
def push_cmd(
    ...
    for slug in targets:
        ...
        if entry.source_type == "npm":
            click.echo(f"{slug}: npm row — nothing to push")
            rejected = True
            continue
        ...
        if direct:
            msg = f"pi-extension({slug}): self-improvement {_utc_iso()}"
...
def _push_via_pr(canonical: Path, entry: object, slug: str, base_ref: str) -> None:
    """Commit local changes on a branch and open a PR (mirrors skill push)."""
    branch = f"pi-ext/self-improvement-{_utc_basic_iso()}-{_slug_for_ref(slug)}"
...
def _open_pr(canonical: Path, branch: str, *, base: str, slug: str) -> str | None:
    ...
    try:
        ...
        return None
    title = f"pi-extension({slug}): self-improvement"
    body = (
