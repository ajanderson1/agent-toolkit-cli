#!/usr/bin/env bash
# audit/demos/agent-claude.sh — reference cell for agent × claude.
#
...
# Projection shape for (claude, agent):
#   Symlink: $CLAUDE_CONFIG_DIR/agents/<slug>.md  ->  $AGENT_TOOLKIT_REPO/agents/<slug>.md
#   This is a FILE-LEVEL symlink (not a directory symlink).
#   The symlink name carries the .md extension.

LINK_DIR="$CLAUDE_CONFIG_DIR/agents"
# TARGET is the symlink path itself (the file symlink with .md suffix).
TARGET="$CLAUDE_CONFIG_DIR/agents/demo-agent.md"
EXPECTED_TARGET="$AGENT_TOOLKIT_REPO/agents/demo-agent.md"
ALLOWLIST="$HOME/.agent-toolkit.yaml"

# ---------- Lifecycle ----------
step "Lifecycle 1/4 — link user claude agent:demo-agent"
run "agent-toolkit-cli link user claude agent:demo-agent"
show "$LINK_DIR"
...
step "Lifecycle 3/4 — unlink user claude agent:demo-agent"
run "agent-toolkit-cli unlink user claude agent:demo-agent"
show "$CLAUDE_CONFIG_DIR/agents"
# FINDING: unlink agent:demo-agent (explicit slug form) reports "Already in sync —
# 0 assets linked, nothing to change." but leaves the symlink in place.
# This is a real CLI gap: explicit-slug unlink doesn't remove an existing symlink.
# The assert below surfaces the bug rather than hiding it.
...
git init -q
run "agent-toolkit-cli --project . link project claude agent:demo-agent"
show ".claude/agents/demo-agent.md"
# Project-scope symlink is also at the file level (with .md suffix).
assert_symlink_exists ".claude/agents/demo-agent.md"
assert_file_contains  ".agent-toolkit.yaml" "demo-agent"
run "agent-toolkit-cli --project . unlink project claude agent:demo-agent"
# FINDING (same as Lifecycle 3/4): project-scope explicit-slug unlink leaves
# the symlink in place — "Already in sync, nothing to change" is incorrect.
assert_no_symlink ".claude/agents/demo-agent.md"
cd - >/dev/null
...
run "agent-toolkit-cli list agent claude"
...
# FINDING: after removing demo-agent from the allowlist, re-running
# `link user claude` reports "Already in sync — 0 assets linked, nothing to
# change." but leaves the previously-created symlink in place.
# Stale symlink pruning on reconcile is not implemented for agent × claude.
assert_no_symlink "$TARGET"
...
#    many other unlinked assets (all WARN) do not trigger a non-zero exit.
