#!/usr/bin/env bash
# audit/demos/skill-pi.sh — reference cell for skill × pi.
#
...
# Projection shape for (pi, skill):
#   User-scope symlink:    $HOME/.pi/agent/skills/<slug>  ->  $AGENT_TOOLKIT_REPO/skills/<slug>/
#   Project-scope symlink: .pi/skills/<slug>              ->  $AGENT_TOOLKIT_REPO/skills/<slug>/
#   Both are DIRECTORY-LEVEL symlinks (not SKILL.md-level).
#   The symlink name is the bare slug with no extension.
#   NOTE: project-scope path is .pi/skills/ (no agent/ subdirectory), unlike user-scope.

LINK_DIR="$HOME/.pi/agent/skills/demo-skill"
# TARGET is the symlink path itself (the directory symlink).
TARGET="$HOME/.pi/agent/skills/demo-skill"
EXPECTED_TARGET="$AGENT_TOOLKIT_REPO/skills/demo-skill"
ALLOWLIST="$HOME/.agent-toolkit.yaml"

# ---------- Lifecycle ----------
step "Lifecycle 1/4 — link user pi skill:demo-skill"
run "agent-toolkit-cli link user pi skill:demo-skill"
show "$LINK_DIR"
...
step "Lifecycle 3/4 — unlink user pi skill:demo-skill"
run "agent-toolkit-cli unlink user pi skill:demo-skill"
show "$HOME/.pi/agent/skills"
...
git init -q
run "agent-toolkit-cli --project . link project pi skill:demo-skill"
show ".pi/skills/demo-skill"
# Project-scope symlink is at .pi/skills/ (no agent/ subdirectory in project scope).
assert_symlink_exists ".pi/skills/demo-skill"
assert_file_contains  ".agent-toolkit.yaml" "demo-skill"
run "agent-toolkit-cli --project . unlink project pi skill:demo-skill"
assert_no_symlink ".pi/skills/demo-skill"
cd - >/dev/null
...
run "agent-toolkit-cli list skill pi"
