#!/usr/bin/env bash
# audit/demos/skill-codex.sh — audit cell for skill × codex.
#
...
# Projection shape for (codex, skill):
#   The CLI writes a SYMLINK at:
#     $CODEX_HOME/skills/<slug>  ->  $CODEX_HOME/.agent-toolkit-cache/skill/<slug>/
#   The symlink does NOT point directly into $AGENT_TOOLKIT_REPO; instead the CLI
#   stages a translated copy of the skill under the CODEX_HOME cache tree first,
#   then symlinks the skills/ entry at the cache path.
#   The cache directory contains SKILL.md (translated frontmatter).
#
# Finding: unlike the claude projection (direct symlink to toolkit repo),
# codex uses an intermediate local cache so that it can translate/reformat
# the skill content for the codex native format.

LINK_DIR="$CODEX_HOME/skills/demo-skill"
TARGET="$CODEX_HOME/skills/demo-skill"
EXPECTED_TARGET="$CODEX_HOME/.agent-toolkit-cache/skill/demo-skill"
ALLOWLIST="$HOME/.agent-toolkit.yaml"

# ---------- Lifecycle ----------
step "Lifecycle 1/4 — link user codex skill:demo-skill"
run "agent-toolkit-cli link user codex skill:demo-skill"
show "$LINK_DIR"
...
step "Lifecycle 3/4 — unlink user codex skill:demo-skill"
run "agent-toolkit-cli unlink user codex skill:demo-skill"
show "$CODEX_HOME/skills"
...
git init -q
run "agent-toolkit-cli --project . link project codex skill:demo-skill"
show ".codex/skills/demo-skill"
# Project-scope: symlink is also at the directory level pointing to local cache.
assert_symlink_exists ".codex/skills/demo-skill"
assert_file_contains  ".agent-toolkit.yaml" "demo-skill"
run "agent-toolkit-cli --project . unlink project codex skill:demo-skill"
assert_no_symlink ".codex/skills/demo-skill"
cd - >/dev/null
...
run "agent-toolkit-cli list skill codex"
...
# 3. Codex-specific: the symlink points to a local cache path, not directly
#    to the toolkit repo. Doctor uses the allowlist+cache to derive expected
#    targets, so replacing the symlink with a regular file is equally invisible
#    under codex as it is under claude.
